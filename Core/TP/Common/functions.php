...
function load_ext_file() {
    ...
    //加载全局动态配置文件
    if(C('LOAD_EXT_CONF')) {
        $configs =  C('LOAD_EXT_CONF');
        if(is_string($configs)) $configs =  explode(',',$configs);
		foreach ($configs as $key=>$config){
			$file = COMMON_CONF_PATH . $config . '.php';
            if(is_file($file)) {
                is_numeric($key)?C(include $file):C($key,include $file);
            }
        }
	...
	//加载全局函数文件
	if(C('LOAD_EXT_FUNC')) {
		$files =  explode(',',C('LOAD_EXT_FUNC'));
        foreach ($files as $file){
            $file   = COMMON_FUNC_PATH.$file.'.php';
            if(is_file($file)) include $file;
        }
	...
	//加载全局类文件
	if(C('LOAD_EXT_LIB')) {
		$files =  explode(',',C('LOAD_EXT_LIB'));
        foreach ($files as $file){
            $file   = COMMON_LIB_PATH.$file.'.class.php';
            if(is_file($file)) include $file;
        }
