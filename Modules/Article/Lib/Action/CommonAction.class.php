...
class CommonAction extends BaseAdmin {
	...
	function _TextCretaeImg($text,$path,$md5id){
        $subpath = substr($md5id,0,2).'/';
	    ...
	    //计算文字所占区域
	    $fontsize = 9;
	    $fontfile = C('PUBLIC_PATH').'/common/font/simsun.ttc';
	    $imgttfbox = ImageTTFBBox($fontsize,0,$fontfile,$text);
	    $width = max($imgttfbox[2],$imgttfbox[4]) - min($imgttfbox[0],$imgttfbox[6]);
	    //$height = max($imgttfbox[1],$imgttfbox[3]) - min($imgttfbox[5],$imgttfbox[6]);
	    //生成图片
	    $im = imagecreate($width,12);
	    $white = ImageColorAllocate($im, 255,255,255);
	    $black = ImageColorAllocate($im, 55,55,55);
	    $imgttftext = ImageTTFText($im,$fontsize, 0, 0, 10, $black, $fontfile, $text);
	    ImagePng($im,$fullpath.$md5id.'.png');
	}
}
