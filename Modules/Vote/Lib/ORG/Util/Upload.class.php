...
class Upload {
    ...
    public function uploadFile($filedata) {
		$this->_indexfilename = rand(10000,99999);
        //获取上传文件基本信息
        ...
        if(!$this->_errno) {
            $newinfo = array(
                'ext' => $info['ext'],
                'realpath' => $dir['realpath'] . $this->_indexpath . $filename,
                'path' => $dir['path'] . $this->_indexpath . $filename,
                'size' => $info['size'],
                'errno' => $this->_errno,
            );
        }else{
    ...
    protected function _createDir($index=true) {
		$this->_indexpath = mb_substr($this->_indexfilename,0,2,'utf-8').'/'.mb_substr($this->_indexfilename,2,2,'utf-8').'/';
        $dir = array();
        $dir['realpath'] = $this->_path.$this->_indexpath;
        $status = true;
    ...
    protected function _createFilename($ext) {
        return $this->_indexfilename . substr(md5(time()), 0, 16) . '.' . $ext;
    }
}
