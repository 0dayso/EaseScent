<?php
// 白酒|香型管理
class B_FlavorAction extends Common2Action {
    /**
     * 初始化
     */
    ...
    public function index(){
        ...
        $count = D()->table('bjk_flavor')->where($map)->count();
        import ("@.ORG.Util.Page");
        $p = new Page($count, 15);
        $list = D()->table('bjk_flavor')->where($map)->order('id desc')->limit($p->firstRow . ',' . $p->listRows)->select();
    ...
    public function add(){
        ...
        if($this->isPost()){
            ...
            if($is_exist = D()->table('bjk_flavor')->where(array('cname'=>$_POST['cname'],'status'=>array('in',array(2,3))))->find()){
            ...
            if(($_POST['ename'] != '') && ($is_exist = D()->table('bjk_flavor')->where(array('ename'=>$_POST['ename'],'status'=>array('in',array(2,3))))->find())){
            ...
            //insert
            $maxid = D()->table('bjk_flavor')->max('id');
            ...
            if(!$is = D()->table('bjk_flavor')->add($insert_data)){
    ...
    public function edit(){
        ...
        if($this->isPost()){
            ...
            if($is_exist = D()->table('bjk_flavor')->where(array('id'=>array('neq',$_POST['id']),'cname'=>$_POST['cname'],'status'=>array('in',array(2,3))))->find()){
            ...
            if(($_POST['ename'] != '') && ($is_exist = D()->table('bjk_flavor')->where(array('id'=>array('neq',$_POST['id']),'ename'=>$_POST['ename'],'status'=>array('in',array(2,3))))->find())){
            ...
            if(false === $is = D()->table('bjk_flavor')->save($update_data)){
                exit(json_encode(array('error'=>1,'msg'=>'修改失败！数据库异常。','_backurl'=>$_backurl)));
            }
            exit(json_encode(array('error'=>0,'msg'=>'修改成功！','_backurl'=>$_backurl)));
        }
        //页面
        if(!$res = D()->table('bjk_flavor')->where(array('id'=>$_GET['id'],'status'=>array('in',array(2,3))))->find()){
