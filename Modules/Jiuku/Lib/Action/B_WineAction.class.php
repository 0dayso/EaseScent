<?php
// 白酒|酒款管理
class B_WineAction extends Common2Action {
    /**
     * 初始化
     */
    ...
    public function index(){
        ...
        if(isset($_GET['flavor_id'])){
            if($_GET['flavor_id'] != 0){
                $map[] = '`flavor_id` = '.$_GET['flavor_id'];
            }else{
                unset($_GET['flavor_id']);
            }
        }
        if(isset($_GET['id'])){
            if($_GET['id'] != ''){
        ...
        if(isset($_GET['barcode'])){
            if($_GET['barcode'] != ''){
                $bracode_res = D()->query('SELECT `fid` FROM `bjk_wine_barcode` WHERE `num` LIKE \'%'.$_GET['barcode'].'%\'');
                $ids = array();
        ...
        $map[] = $status_map;
        //list&page
        $count = D()->table('bjk_wine')->where(implode(' AND ',$map))->count();
        import ("@.ORG.Util.Page");
        $p = new Page($count, 15);
        $list = D()->table('bjk_wine')->where(implode(' AND ',$map))->order('id desc')->limit($p->firstRow . ',' . $p->listRows)->select();
        $region_list = D()->table('bjk_region')->field('id,cname,ename')->where(array('status'=>3))->select();
        $region_nlist = array();
        foreach($region_list as $val){
            $region_nlist[$val['id']] = $val;
        }
        $flavor_list = D()->table('bjk_flavor')->field('id,cname,ename')->where(array('status'=>3))->select();
        $flavor_nlist = array();
        foreach($flavor_list as $val){
            $flavor_nlist[$val['id']] = $val;
        }
        foreach($list as $key=>$val){
            $list[$key]['region_res'] = $region_nlist[$val['region_id']];
            $list[$key]['flavor_res'] = $flavor_nlist[$val['flavor_id']];
        }
        $this->assign('list', $list);
        $this->assign("page", $p->show());
        $this->assign('region_list', $region_list);
        $this->assign('flavor_list', $flavor_list);
    ...
    public function add(){
        ...
        if($this->isPost()){
            ...
            if($_POST['type'] == 'ini_barcode'){
                if($is_exist = D()->query('SELECT `fid` FROM `bjk_wine_barcode` WHERE `num` = '.$_POST['barcode'])){
            ...
            if($is_exist = D()->table('bjk_wine')->where(array('cname'=>$_POST['cname'],'status'=>array('in',array(2,3))))->find()){
            ...
            if(($_POST['ename'] != '') && ($is_exist = D()->table('bjk_wine')->where(array('ename'=>$_POST['ename'],'status'=>array('in',array(2,3))))->find())){
            ...
            foreach($_POST['barcodes'] as $barcode){
                if($is_exist = D()->query('SELECT B.`id` FROM `bjk_wine_barcode` A INNER JOIN `bjk_wine` B ON B.`id` = A.`fid` WHERE A.`num` = '.$barcode)){
            ...
            if(!$is = D()->table('bjk_wine')->add($insert_data)){
            ...
            D()->table('bjk_wine_image')->addall($img_data);
            //条形码
            $barcode_data = array();
            ...
            D()->table('bjk_wine_barcode')->addall($barcode_data);
            //品牌
            $brand_data = array();
    ...
    public function edit(){
        ...
        if($this->isPost()){
            ...
            //INI barcode判断
            if($_POST['type'] == 'ini_barcode'){
                if($is_exist = D()->query('SELECT `fid` FROM `bjk_wine_barcode` WHERE `num` = '.$_POST['barcode'].' AND `fid` != '.$_POST['id'])){
                    exit(json_encode(array('error'=>1,'msg'=>'增加失败！“条形码：'.$_POST['barcode'].'”重复录入，重复ID：' . $is_exist[0]['fid'] . '。<a href="'.Url('index').'&id=' . $is_exist[0]['fid'] . '" target="_blank">去看看</a>')));
                }else{
                    exit(json_encode(array('error'=>0)));
                }
            ...
            if($is_exist = D()->table('bjk_wine')->where(array('id'=>array('neq',$_POST['id']),'cname'=>$_POST['cname'],'status'=>array('in',array(2,3))))->find()){
            ...
            if(($_POST['ename'] != '') && ($is_exist = D()->table('bjk_wine')->where(array('id'=>array('neq',$_POST['id']),'ename'=>$_POST['ename'],'status'=>array('in',array(2,3))))->find())){
            ...
            foreach($_POST['barcodes'] as $barcode){
                if($is_exist = D()->query('SELECT B.`id` FROM `bjk_wine_barcode` A INNER JOIN `bjk_wine` B ON B.`id` = A.`fid` WHERE A.`num` = '.$barcode.' AND A.`fid` != '.$_POST['id'])){
            ...
            if(false === $is = D()->table('bjk_wine')->save($update_data)){
                exit(json_encode(array('error'=>1,'msg'=>'修改失败！数据库异常。','_backurl'=>$_backurl)));
            }
            //图片
            D()->table('bjk_wine_image')->where(array('fid'=>$_POST['id']))->delete();
            $img_data = array();
            ...
            D()->table('bjk_wine_image')->addall($img_data);
            //条形码
            D()->table('bjk_wine_barcode')->where(array('fid'=>$_POST['id']))->delete();
            $barcode_data = array();
            foreach($_POST['barcodes'] as $val){
                $barcode_data[] = array(
                    'fid' => $_POST['id'],
        ...
        //页面
        if(!$res = D()->table('bjk_wine')->where(array('id'=>$_GET['id'],'status'=>array('in',array(2,3))))->find()){
        ...
        $res['flavor_res'] = $flavor_nlist[$res['flavor_id']];
        $res['img_res'] = D()->table('bjk_wine_image')->where(array('fid'=>$_GET['id']))->select();
        $res['barcode_res'] = D()->table('bjk_wine_barcode')->where(array('fid'=>$_GET['id']))->select();
