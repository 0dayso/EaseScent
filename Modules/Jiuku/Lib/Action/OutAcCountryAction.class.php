 ...
 * 权限控制外国家方法
 */
class OutAcCountryAction extends Action {
    /**
     * 初始化
     */
	 ...
	 *
	 */
	function ajaxKeywordSearchCountry(){
		if(!isset($_POST['kw'])){
		...
		$result = array();
		if(preg_match("/^(-|\+)?\d+$/",$kw)){
            $res = D('Country')->field('id,fname,cname')->where(array_merge($map,array('id'=>$kw)))->find();
	...
	public function hit(){
		...
		if($Redis = $this->linkRedis()){
			$res = $Redis->hgetall('jk:country_hit:'.$id.':');
			if(!$res){
				$Redis->hmset('jk:country_hit:'.$id.':',array('add'=>1,'all'=>1,'time'=>time()));
			}else{
				$Redis->hmset('jk:country_hit:'.$id.':',array('add'=>$res['add']+1,'all'=>$res['all']+1,'time'=>$res['time']));
				if((time()-$res['time']) > 7200){
					D()->query('UPDATE `jiuku_country` SET `hit` = `hit` +'.($res['add']+1).' WHERE `id` = '.$id);
					$s_res = D('Country')->where(array('id'=>$id))->getfield('hit');
					$Redis->hmset('jk:country_hit:'.$id.':',array('add'=>0,'all'=>$s_res,'time'=>time()));
				}
			}
			$this->echo_exit(array('errorCode'=>0,'errorStr'=>'','result'=>$res['all']+1));
		}else{
			D()->query('UPDATE `jiuku_country` SET `hit` = `hit` +1 WHERE `id` = '.$id);
			$res = D('Country')->where(array('id'=>$id))->getfield('hit');
