 ...
 * 葡萄品种颜色管理
 */
class GrapecolorAction extends CommonAction {
	
	/**
	 * 列表
	 */
	public function index() {
		$list = $this->_list(D('Grapecolor'));
	...
	public function add() {
		if($this->isPost()) {
			$artid = $this->_insert(D('Grapecolor'));
			$this->_jumpGo('葡萄品种颜色添加成功', 'succeed', Url('index'));
		}
		$this->display();
	}
	...
	public function edit() {
        ...
        if($this->isPost()) {
            $this->_update($model);
			$this->_jumpGo('葡萄品种颜色编辑成功', 'succeed', Url('index'));
        }
        $grape_color_res = $model->where(array('id' => $id))->find();
        $this->assign('vo', $grape_color_res);
        $this->display();
	}
    ...
    public function chgStatus() {
		...
		$this->_update(D('Grapecolor'),$data);
		$this->_jumpGo('ID为'.$aid.'的葡萄品种颜色状态更改成功', 'succeed', Url('index'));
    }
}
