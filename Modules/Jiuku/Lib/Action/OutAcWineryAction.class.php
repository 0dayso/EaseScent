 ...
 * 权限控制外酒庄方法
 */
class OutAcWineryAction extends Action {
    /**
     * 初始化
     */
	 ...
	 *
	 */
	function ajaxKeywordSearchWinery(){
		if(!isset($_POST['kw'])){
		...
		$result = array();
		if(preg_match("/^(-|\+)?\d+$/",$kw)){
            $res = D('Winery')->field('id,fname,cname')->where(array_merge($map,array('id'=>$kw)))->find();
	...
	public function hit(){
		...
		if($Redis = $this->linkRedis()){
			$res = $Redis->hgetall('jk:winery_hit:'.$id.':');
			if(!$res){
				$Redis->hmset('jk:winery_hit:'.$id.':',array('add'=>1,'all'=>1,'time'=>time()));
			}else{
				$Redis->hmset('jk:winery_hit:'.$id.':',array('add'=>$res['add']+1,'all'=>$res['all']+1,'time'=>$res['time']));
				if((time()-$res['time']) > 7200){
					D()->query('UPDATE `jiuku_winery` SET `hit` = `hit` +'.($res['add']+1).' WHERE `id` = '.$id);
					$s_res = D('Winery')->where(array('id'=>$id))->getfield('hit');
					$Redis->hmset('jk:winery_hit:'.$id.':',array('add'=>0,'all'=>$s_res,'time'=>time()));
				}
			}
			$this->echo_exit(array('errorCode'=>0,'errorStr'=>'','result'=>$res['all']+1));
		}else{
			D()->query('UPDATE `jiuku_winery` SET `hit` = `hit` +1 WHERE `id` = '.$id);
			$res = D('Winery')->where(array('id'=>$id))->getfield('hit');
