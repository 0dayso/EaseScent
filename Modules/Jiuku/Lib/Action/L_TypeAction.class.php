<?php
// 洋酒|类型管理
class L_TypeAction extends Common2Action {
    /**
     * 初始化
     */
    ...
    public function index(){
        ...
        $count = D()->table('ljk_type')->where($map)->count();
        import ("@.ORG.Util.Page");
        $p = new Page($count, 15);
        $list = D()->table('ljk_type')->where($map)->order('id desc')->limit($p->firstRow . ',' . $p->listRows)->select();
    ...
    public function add(){
        ...
        if($this->isPost()){
            ...
            if($is_exist = D()->table('ljk_type')->where(array('cname'=>$_POST['cname'],'status'=>array('in',array(2,3))))->find()){
            ...
            if(($_POST['ename'] != '') && ($is_exist = D()->table('ljk_type')->where(array('ename'=>$_POST['ename'],'status'=>array('in',array(2,3))))->find())){
            ...
            if(!$is = D()->table('ljk_type')->add($insert_data)){
    ...
    public function edit(){
        ...
        if($this->isPost()){
            ...
            if($is_exist = D()->table('ljk_type')->where(array('id'=>array('neq',$_POST['id']),'cname'=>$_POST['cname'],'status'=>array('in',array(2,3))))->find()){
            ...
            if(($_POST['ename'] != '') && ($is_exist = D()->table('ljk_type')->where(array('id'=>array('neq',$_POST['id']),'ename'=>$_POST['ename'],'status'=>array('in',array(2,3))))->find())){
            ...
            if(false === $is = D()->table('ljk_type')->save($update_data)){
                exit(json_encode(array('error'=>1,'msg'=>'修改失败！数据库异常。','_backurl'=>$_backurl)));
            }
            exit(json_encode(array('error'=>0,'msg'=>'修改成功！','_backurl'=>$_backurl)));
        }
        //页面
        if(!$res = D()->table('ljk_type')->where(array('id'=>$_GET['id'],'status'=>array('in',array(2,3))))->find()){
