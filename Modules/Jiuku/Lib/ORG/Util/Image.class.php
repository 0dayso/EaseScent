...
class Image {
    ...
    static function thumb($image, $thumbname, $type='', $maxWidth=200, $maxHeight=50, $interlace=true) {
        ...
        if(!is_dir(dirname($thumbname))){
            ...
            @touch(dirname($thumbname).'/index.html');
        }
        // 获取原图信息
    ...
    /**
      +----------------------------------------------------------
     * 生成缩略图(小于等于缩略图宽高分辨率，自适应压缩)
      +----------------------------------------------------------
     * @static
     * @access public
      +----------------------------------------------------------
     * @param string $infile    原图
     * @param string $outfile   缩略图
     * @param string $thumb_w   缩略图宽度
     * @param string $thumb_h   缩略图高度
      +----------------------------------------------------------
     * @return void
      +----------------------------------------------------------
     */
    static function thumb1($infile, $outfile, $thumb_w=0, $thumb_h=0){
        if(!is_file($infile) || $infile == $outfile || (intval($thumb_w) <= 0 && intval($thumb_h) <= 0))    return false;
        $tempArr = explode(".", basename($infile));
        $ext = strtolower(trim(array_pop($tempArr)));
        if(!in_array($ext,array('gif','jpg','jpeg','png')))    return false;
        $imgType = array(1=>'gif', 2=>'jpeg', 3=>'png');
        if(!list($width, $height, $type) = getimagesize($infile))    return false;
        $funcCreate = "imagecreatefrom".$imgType[$type];
        $funcOut = "image".$imgType[$type];
        if(intval($thumb_w) <= 0){
            $w_h = 'h';
        }elseif(intval($thumb_h <= 0)){
            $w_h = 'w';
        }else{
            $w_h = ($thumb_w/$width > $thumb_h/$height) ? 'h' : 'w';
        }
        if($w_h == 'w'){
            $w_h_percent = $thumb_w/$width;
            $z_width = $thumb_w;
            $z_height = ceil($height*$w_h_percent);
        }else{
            $w_h_percent = $thumb_h/$height;
            $z_height = $thumb_h;
            $z_width = ceil($width*$w_h_percent);
        }
        $thumb = imagecreatetruecolor($z_width, $z_height);
        imagefill($thumb,0,0,imagecolorAllocate($thumb,255,255,255));
        $img = $funcCreate($infile);
        imagecopyresampled($thumb, $img, 0, 0, 0, 0, $z_width, $z_height, $width, $height);
        if(!is_dir(dirname($outfile))){
            mkdir(dirname($outfile), 0777 ,true);
            @touch(dirname($outfile).'/index.html');
        }
     ...
     * @param string $thumb_w   缩略图宽度
     * @param string $thumb_h   缩略图高度
     * @param string $offset_w  压缩宽度偏移量(-100 ~ 100)
     * @param string $offset_h  压缩高度偏移量(-100 ~ 100)
      +----------------------------------------------------------
     * @return void
      +----------------------------------------------------------
     */
    static function thumb2($infile, $outfile, $thumb_w=0, $thumb_h=0, $offset_w=0, $offset_h=0){
        if(!is_file($infile) || $infile == $outfile || intval($thumb_w) <= 0 || intval($thumb_h) <= 0 || intval($offset_w) > 100 || intval($offset_w) < -100 || intval($offset_h) > 100 || intval($offset_h) < -100)    return false;
        $tempArr = explode(".", basename($infile));
        $ext = strtolower(trim(array_pop($tempArr)));
        if(!in_array($ext,array('gif','jpg','jpeg','png')))    return false;
        $imgType = array(1=>'gif', 2=>'jpeg', 3=>'png');
        ...
        $funcCreate = "imagecreatefrom".$imgType[$type];
        $funcOut = "image".$imgType[$type];
        $w_h = ($width/$thumb_w > $height/$thumb_h) ? 'w' : 'h';
        if($w_h == 'w'){
            $w_h_percent = $height/$thumb_h;
            $z_height = $thumb_h;
            $z_width = ceil($width/$w_h_percent);
            $x = ceil(($z_width-$thumb_w)/2*$w_h_percent*($offset_w/100+1));
            $y = 0;
        }else{
            $w_h_percent = $width/$thumb_w;
            $z_width = $thumb_w;
            $z_height = ceil($height/$w_h_percent);
            $x = 0;
            $y = ceil(($z_height-$thumb_h)/2*$w_h_percent*($offset_h/100+1));
        }
     ...
     * @return void
      +----------------------------------------------------------
     */
    static function thumb3($infile, $outfile, $thumb_w=0, $thumb_h=0, $offset_w=0, $offset_h=0){
        if(!is_file($infile) || $infile == $outfile || intval($thumb_w) <= 0 || intval($thumb_h) <= 0 || intval($offset_w) > 100 || intval($offset_w) < -100 || intval($offset_h) > 100 || intval($offset_h) < -100)    return false;
        $tempArr = explode(".", basename($infile));
        $ext = strtolower(trim(array_pop($tempArr)));
        if(!in_array($ext,array('gif','jpg','jpeg','png')))    return false;
        ...
        if($w_h == 'w'){
            ...
            $z_height = ceil($height/$w_h_percent);
            $x = 0;
            $y = floor(($thumb_h-$z_height)/2*($offset_w/100+1));
        }else{
            $w_h_percent = $height/$thumb_h;
            $z_height = $thumb_h;
            $z_width = ceil($width/$w_h_percent);
            $x = floor(($thumb_w-$z_width)/2*($offset_h/100+1));
            $y = 0;
        }
        $thumb = imagecreatetruecolor($thumb_w, $thumb_h);
        imagefill($thumb,0,0,imagecolorAllocate($thumb,248,248,248));
        $img = $funcCreate($infile);
