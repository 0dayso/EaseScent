...
class AnxinjieSmsSender{
    ...
    var $use_curl                    = false;
	//调用例子，在最下面，仅供参考调用
	public function sendsms($username, $password, $mobiles,$content,$sendtime='',$subid='')
	{
		if($sendtime=='')
		{
		...
		$apidata="name=$username&pass=$password&subid=$subid&mobiles=$mobiles&content=$content&sendtime=$sendtime";
		//echo $apidata;
		$apiurl = "http://111.13.56.193:9003/";
		//echo $apiurl;
		$ret= $this->request($apiurl,$apidata);
		$ret["body"]=mb_convert_encoding($ret["body"], 'utf-8' ,'gb2312');
		if($ret && strpos($ret["body"],"<reqCode>00</reqCode>")>0) {
