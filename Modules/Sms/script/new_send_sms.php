...
if(!extension_loaded('redis'))
{
	...
	dl($prefix.'igbinary.'.PHP_SHLIB_SUFFIX);
	dl($prefix.'redis.'.PHP_SHLIB_SUFFIX);
}
...
$redis_cfg = array(
        'host' => REDIS_HOST,
        'port' => REDIS_PORT,
        'key'  => NEW_REDIS_QUEUE_PREFIX,
        );

require_once('db.vdr.php');
require_once('AnxinjieSmsSender.php');
$time_c = 0;
    ...
    if($pid) 
	{
        ...
        while(true) {
            ...
            if ( $message ) {
                ...
                // call Bayou API 
                $sender=new AnxinjieSmsSender();
                $count_tel_arr = count($data['tel_list']);
                $str_tel_list = implode(',', $data['tel_list']);
                $rec = $sender->sendsms($rst['account'], $rst['password'], $str_tel_list, $data['content']);
                //$rec = array('status'=>1);
